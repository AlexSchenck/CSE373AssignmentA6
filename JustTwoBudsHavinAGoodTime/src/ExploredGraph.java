...
public class ExploredGraph {
	...
	ArrayList<Operator> operations; // list of all operators on 3 pegs
	
	public ExploredGraph() {
		Ve = new LinkedHashSet<Vertex>();
		Ee = new LinkedHashSet<Edge>();
		path = new HashMap<Vertex, Vertex>(); // path generated by last search
		
		// define the list of 6 possible operators
	...
	// clear explored vertices and edges, add initial vertex examined vertices
	...
	// return number of vertices explored
	public int nvertices() {
		return Ve.size();
	}
	
	// return number of edges explored
	public int nedges() {
		return Ee.size();
	}
	
	// run a depth-first search starting with vi, ending at vj
	public void dfs(Vertex vi, Vertex vj) {
		// To-do list of vertices to process
		Stack<Vertex> fringe = new Stack<Vertex>();
		fringe.push(vi);
		
		// run until fringe is empty
		while(!fringe.isEmpty()) {
			Vertex current = fringe.pop();
			
			// do not re-visit any vertices
			if(!Ve.contains(current)) {
				Ve.add(current);
				
				// run all possible operations on this vertex, process all resulting vertices
	...
	// run a bredth-first search starting with vi, ending at vj
	...
	// returns path to vi as ArrayList of vertices
	...
	public ArrayList<Vertex> shortestPath(Vertex vi, Vertex vj) {
		// clear current search, run bfs
		initialize(vi);
		bfs(vi, vj);
		
		// iterate through map if it contains vj, and construct ArrayList path
		if(Ve.contains(vj)) {
			ArrayList<Vertex> result = new ArrayList<Vertex>();
